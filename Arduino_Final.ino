...
// Sorts and returns the median value of a five element array.(Used to withstand the errors produced by the ultrasonic sensors)
...
{   
    ...
    float lconst = ldist1;
     ...
     while(ldist1<=5)    // Should turn a little to its right..as it's close to the left wall. 
     {   
...
As our maze was fixed and the last turn was a left turn, we have used the end condition, so as to let the bot know that we have reached the end, the end condition as 7 left turns.
I highly recommend to  use any other end condition such as using a black stip with Ir sensor or any other such. 

*/

void loop()    // put your main code here, to run repeatedly
 {       //end condition ... here it is no. of left turns*(as the last turn was left turn)
         if(nlr==7)   // to indicate the end .. we have used the no of left turns in the maze ... use of IR sensor is recommended.
                      {  
                        found=true;   // Reached the end. 

                        for(int i=0;i<=2;i++){Serial.print(dir[i]);}
                        i=-1;j=0; nlr=0;      // Back to the start again..
  ...
  {  
   if((rsensor==false)&&(frontdist()>=40))    // If both front and right are open..
    { 
     ...
     if((found==true)&(dir[i]!=0))   // After reaching the end ... verifying  with the simplified array.
                                  {
                                    rightturn();return;
                                  }
   else
     {  
      ...
      timefr = tfr + 65*nf; // For a non-constant potential source .. we need to increse the time. 
      nf=nf+1; // as time increases we have add more time ... thus incrementing nf.
...
//for a right turn.. 
   ...
   {   
     ...
     while( abs(frontdist()-prevfdist)<=(prevfdist/2)-2) {gofront();delay(300);if(frontdist()<=4.5){break;}}
...
else
 {
   ...
   j=j+1;
    ...
    // Verifying with the simplified array...
